...
public sealed class WorkspaceScope : IDisposable
{
    ...
    /// <summary>
    /// Like <see cref="Begin"/>, but returns null instead of throwing when the path is blank or not an
    /// existing directory — the shape the endpoints need to turn a bad path into a 400/error event.
    /// </summary>
    public static WorkspaceScope? TryBegin(string? root)
    {
        try
        {
            return Begin(root);
