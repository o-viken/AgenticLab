...
internal sealed class HostingStory
{
    ...
    [
        new("local", "Personal runtime",
            ...
            "Your computer runs the harness. It stops when the process stops or the machine sleeps.",
            "A remote model API, or a compatible local model. A local harness does not keep data local if it sends context to a remote model.",
        ...
        new("service", "Your own service",
            ...
            "Your harness runs on company infrastructure or cloud compute: for example, a container, application service or virtual machine.",
            "Your code calls an approved model endpoint, or separately operated inference. Hosting the harness does not host the model.",
